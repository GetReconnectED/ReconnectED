...
/**
 * Composable for rendering the Dashboard screen interface. The Dashboard includes components for
 * displaying user statistics such as screen time, days active, as well as navigation options for
 * additional features like AI Assistant and App Usage Limits.
 *
...
fun Dashboard(
    ...
    var canDrawOverlay by remember {
        mutableStateOf(
                com.getreconnected.reconnected.core.util.AppLimitPermissionHelper.canDrawOverlays(
                        context
                )
        )
    }
    ...
    val (selectedQuote, setSelectedQuote) =
            remember { mutableStateOf(QuotesManager.getQuotes(context).random()) }

    val screenTimeMillis by
            produceState(initialValue = -1L, key1 = hasAllPermissions) {
                if (hasAllPermissions) {
                    while (true) {
    ...
    Column(
        ...
        if (screenTimeMillis > 0) {
            // Calculate CO2e emissions
            // Using average rate of 0.9 gCO2e per minute across all categories
            val screenTimeMinutes = screenTimeMillis / (1000 * 60).toDouble()
            val co2eGrams = screenTimeMinutes * 0.9
            val equivalentKm = (co2eGrams / 400) * 1.60934
            ...
            ElevatedCard(
                ...
                Row(
                    ...
                    Icon(
                            painter = painterResource(R.drawable.days_active),
                    ...
                    Column(modifier = Modifier.weight(1f)) {
                        ...
                        Text(
                                text =
                                        buildString {
                                            append("Your ")
                                            append("%.1f".format(co2eGrams))
                                            append(
                                                    "g CO₂e from app usage today is equivalent to driving "
                                            )
                                            append("%.2f".format(equivalentKm))
                                            append(" km in a typical passenger vehicle.")
                                        },
                                style =
                                        TextStyle(
                                                fontFamily = interDisplayFamily,
...
/**
 * Displays a greeting text based on the time of day and the provided name. The greeting adapts to
 * different times of the day (morning, afternoon, evening, night) and incorporates the user's name
 * into the message.
 *
 ...
 *
 * @param modelProducer A producer for the Cartesian chart model, which provides the data and
    ...
    CartesianChartHost(
            chart =
                    rememberCartesianChart(
                            ...
                            layerPadding = {
                                cartesianLayerPadding(scalableStart = 8.dp, scalableEnd = 8.dp)
                            },
                    ),
...
/**
 * A composable function that displays a bar chart representing the average screen time per day over
 * a week. It dynamically updates its data by leveraging a chart model producer.
 *
 * @param modifier Modifier to customize the layout and styling of the chart's container.
 */
@Composable
...
fun WeeklyAvgScreenTimeChart(modifier: Modifier = Modifier) {
    ...
    LaunchedEffect(Unit) { screenTimeViewModel.loadWeeklyUsageStats() }
...
fun DashboardPermissionRequestScreen(
    ...
    Column(
        ...
        Text(
                text =
                        "To help you manage screen time and enforce app limits, we need special permissions:",
                style =
                        TextStyle(
                                fontFamily = interDisplayFamily,
                                fontSize = 16.sp,
        ...
        if (!hasUsageStats || !canDrawOverlay) {
            Text(
                    text =
                            "Please tap the buttons above to grant permissions. The app will continue once all permissions are granted.",
                    style =
                            TextStyle(
                                    fontFamily = interDisplayFamily,
                                    fontSize = 14.sp,
